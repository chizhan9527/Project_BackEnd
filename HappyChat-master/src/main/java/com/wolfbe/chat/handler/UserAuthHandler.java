...
public class UserAuthHandler extends SimpleChannelInboundHandler<Object> {
    ...
    private void handleWebSocket(ChannelHandlerContext ctx, WebSocketFrame frame) throws IOException, TimeoutException {
        ...
        int member_id=json.getInteger("member_id");
        String member_name=json.getString("member_name");
        switch (code) {
...
//                UserInfoManager.sendPong(ctx.channel());
                ...
                return;
            case ChatCode.AUTH_CODE:    //新人加入群聊
                //boolean isSuccess = UserInfoManager.saveUser(channel, json.getString("nick"));
                int group_id1=json.getInteger("group_id");
                ChatGroup find_group=UserInfoManager.getGroup(group_id1);
                //不为空说明能找到对应的社团,将其加入到对应社团
                if(find_group!=null){
                    find_group.add_user(UserInfoManager.saveUser(channel,member_name,member_id));
                }
                else
                    return;
                return;
            case ChatCode.MESS_CODE: //普通的消息留给MessageHandler处理
                break;
            case ChatCode.GROUP_NEW:  //表示建立新群
                int group_id=json.getInteger("group_id");
                String group_name=json.getString("group_name");
                ChatGroup new_group=new ChatGroup(group_name,group_id,member_id,member_name,channel);
                UserInfoManager.addGroup(group_id,new_group);
                return;
            default:
